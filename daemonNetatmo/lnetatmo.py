...
# To be able to have a program accessing your netatmo data, 83ff45a1e3023c8b45bf"   # Your client ID from Netatmo app registration at http://dev.netatmo.com/dev/listapps
_CLIENT_ID     = "55cb83ff45a1e3023c8b45bf" 
_CLIENT_SECRET = "mNGurMP4d6IBHhzyukT2ON8HowjnudjE"   # Your client app secret   '     '
_USERNAME      = "matteo.danieletto@gmail.com"   # Your netatmo account username
_PASSWORD      = "Esedra#84"   # Your netatmo account password
...
#MATTEO ADD
_GETPUBLIC      = _BASE_URL + "api/getpublicdata"
...
class StationsList:
    ...
    def getMeasureZone(self, lat_ne, lon_ne, lat_sw, lon_sw):
    
        postParams = { "access_token" : self.getAuthToken }
        postParams['lat_ne'] = lat_ne;
        postParams['lon_ne'] = lon_ne;
        postParams['lat_sw'] = lat_sw;
        postParams['lon_sw'] = lon_sw;
        
        return postRequest(_GETPUBLIC, postParams)
...
class DeviceList:
    ...
    def stationByNameMatteo(self, station=None):
        ...
        for i,s in self.stations.items():                            
            ...
            city        = place['city']            
            MAC_ADDRESS=s['_id']
            dashboard=s['dashboard_data']
            station=[MAC_ADDRESS,dashboard['time_utc'],dashboard['Temperature'],dashboard['CO2'],dashboard['Humidity'],dashboard['Pressure'],dashboard['AbsolutePressure'],dashboard['Noise'],city,location[1],location[0]]
            stationUnderControl.append(station)
        
    
    
        if stationUnderControl != None:
            return stationUnderControl
        else:
            return None
# type
    def moduleByNameMatteo(self, station=None):
        
        for m in self.modules:
            mod = self.modules[m]
            
            if mod['main_device'] == station and mod['type'] == 'NAModule4' :
                # print m
                # print mod
                dashboard=mod['dashboard_data']
                module=[mod['main_device'], mod['_id'],dashboard['time_utc'],dashboard['Temperature'],dashboard['CO2'],dashboard['Humidity']]
                return module
